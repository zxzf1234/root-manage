...
import com.xiyu.service.framework.job.core.util.CronUtils;
import com.xiyu.service.vo.infra.data.job.job.*;
import com.xiyu.service.convert.infra.data.JobConvert;
import com.xiyu.service.model.infra.job.InfraJob;
import com.xiyu.service.service.infra.data.JobService;
...
public class JobController {
    ...
    @PreAuthorize("@ss.hasPermission('infra:data:job:create')")
    public CommonResult<String> createJob(@Valid @RequestBody JobCreateReqVO createReqVO)
    ...
    @PreAuthorize("@ss.hasPermission('infra:data:job:update')")
    ...
    @PreAuthorize("@ss.hasPermission('infra:data:job:update')")
    public CommonResult<Boolean> updateJobStatus(@RequestParam(value = "id") UUID id, @RequestParam("status") Integer status)
            throws SchedulerException {
    ...
    @Parameter(name = "id", description = "编号", required = true, example = "1024")
	@PreAuthorize("@ss.hasPermission('infra:data:job:delete')")
    public CommonResult<Boolean> deleteJob(@RequestParam("id") UUID id)
            throws SchedulerException {
    ...
    @Parameter(name = "id", description = "编号", required = true, example = "1024")
    @PreAuthorize("@ss.hasPermission('infra:data:job:trigger')")
    public CommonResult<Boolean> triggerJob(@RequestParam("id") UUID id) throws SchedulerException {
    ...
    public CommonResult<JobRespVO> getJob(@RequestParam("id") UUID id) {
        InfraJob job = jobService.getJob(id);
    ...
    @PreAuthorize("@ss.hasPermission('infra:data:job:query')")
    public CommonResult<List<JobRespVO>> getJobList(@RequestParam("ids") Collection<UUID> ids) {
        List<InfraJob> list = jobService.getJobList(ids);
    ...
    public CommonResult<PageResult<JobRespVO>> getJobPage(@Valid JobPageReqVO pageVO) {
        PageResult<InfraJob> pageResult = jobService.getJobPage(pageVO);
    ...
    public void exportJobExcel(@Valid JobExportReqVO exportReqVO,
        ...
        List<InfraJob> list = jobService.getJobList(exportReqVO);
        // 导出 Excel
    ...
    @PreAuthorize("@ss.hasPermission('infra:data:job:query')")
    public CommonResult<List<LocalDateTime>> getJobNextTimes(@RequestParam("id") UUID id,
                                                   @RequestParam(value = "count", required = false, defaultValue = "5") Integer count) {
        InfraJob job = jobService.getJob(id);
        if (job == null) {
            return success(Collections.emptyList());
        }
        return success(CronUtils.getNextTimes(job.cronExpression(), count));
    }
