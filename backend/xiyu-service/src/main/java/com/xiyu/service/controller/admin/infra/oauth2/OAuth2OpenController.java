...
import com.xiyu.service.util.http.HttpUtils;
...
import com.xiyu.service.convert.infra.oauth2.OAuth2OpenConvert;
import com.xiyu.service.enums.infra.oauth2.InfraOauth2GrantTypeEnum;
...
import com.xiyu.service.service.infra.oauth2.Oauth2ApproveService;
import com.xiyu.service.service.infra.oauth2.Oauth2ClientService;
import com.xiyu.service.service.infra.oauth2.Oauth2GrantService;
...
import static com.xiyu.service.framework.exception.enums.GlobalErrorCodeConstants.BAD_REQUEST;
import static com.xiyu.service.framework.exception.util.ServiceExceptionUtil.exception0;
import static com.xiyu.service.framework.web.web.core.pojo.CommonResult.success;
import static com.xiyu.service.util.collection.CollectionUtils.convertList;
...
public class OAuth2OpenController {
    ...
    public CommonResult<OAuth2OpenAccessTokenRespVO> postAccessToken(HttpServletRequest request,
        ...
        SystemOauth2Client client = oauth2ClientService.validOAuthClientFromCache(clientIdAndSecret[0], clientIdAndSecret[1],
                grantType, scopes, redirectUri);

        // 2. 根据授权模式，获取访问令牌
        SystemOauth2AccessToken accessTokenDO;
        switch (grantTypeEnum) {
            case AUTHORIZATION_CODE:
                accessTokenDO = oauth2GrantService.grantAuthorizationCodeForAccessToken(client.clientId(), code, redirectUri, state);
                break;
            case PASSWORD:
                accessTokenDO = oauth2GrantService.grantPassword(username, password, client.clientId(), scopes);
                break;
            case CLIENT_CREDENTIALS:
                accessTokenDO = oauth2GrantService.grantClientCredentials(client.clientId(), scopes);
                break;
            case REFRESH_TOKEN:
                accessTokenDO = oauth2GrantService.grantRefreshToken(refreshToken, client.clientId());
                break;
            default:
    ...
    public CommonResult<Boolean> revokeToken(HttpServletRequest request,
        ...
        // 删除访问令牌
        return success(oauth2GrantService.revokeToken(client.clientId(), token));
    }
    ...
    public CommonResult<OAuth2OpenCheckTokenRespVO> checkToken(HttpServletRequest request,
        ...
        // 校验令牌
        SystemOauth2AccessToken accessTokenDO = oauth2TokenService.checkAccessToken(token);
        Assert.notNull(accessTokenDO, "访问令牌不能为空"); // 防御性检查
    ...
    public CommonResult<OAuth2OpenAuthorizeInfoRespVO> authorize(@RequestParam("clientId") String clientId) {
        ...
        // 1. 获得 Client 客户端的信息
        SystemOauth2Client client = oauth2ClientService.validOAuthClientFromCache(clientId);
        // 2. 获得用户已经授权的信息
        List<SystemOauth2Approve> approves = oauth2ApproveService.getApproveList(getLoginUserId(), getUserType(), clientId);
        // 拼接返回
    ...
    private String getImplicitGrantRedirect(Long userId, SystemOauth2Client client,
                                            List<String> scopes, String redirectUri, String state) {
        // 1. 创建 access token 访问令牌
        SystemOauth2AccessToken accessTokenDO = oauth2GrantService.grantImplicit(userId, getUserType(), client.clientId(), scopes);
        Assert.notNull(accessTokenDO, "访问令牌不能为空"); // 防御性检查
        // 2. 拼接重定向的 URL
        // noinspection unchecked
        return OAuth2Utils.buildImplicitRedirectUri(redirectUri, accessTokenDO.accessToken(), state, accessTokenDO.expiresTime(),
                scopes, JsonUtils.parseObject(client.additionalInformation(), Map.class));
    }

    private String getAuthorizationCodeRedirect(Long userId, SystemOauth2Client client,
                                                List<String> scopes, String redirectUri, String state) {
        // 1. 创建 code 授权码
        String authorizationCode = oauth2GrantService.grantAuthorizationCodeForCode(userId, getUserType(), client.clientId(), scopes,
                redirectUri, state);
        // 2. 拼接重定向的 URL
