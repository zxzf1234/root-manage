...
import cn.iocoder.yudao.service.service.infra.sensitiveword.SensitiveWordService;
...
@Tag(name = "管理后台 - 敏感词")
@RestController
@RequestMapping("/infra/data/sensitive-word")
@Validated
public class SensitiveWordController {
    ...
    @PreAuthorize("@ss.hasPermission('infra:data:sensitive-word:create')")
    ...
    @PreAuthorize("@ss.hasPermission('infra:data:sensitive-word:update')")
    ...
    @PreAuthorize("@ss.hasPermission('infra:data:sensitive-word:delete')")
    ...
    @PreAuthorize("@ss.hasPermission('infra:data:sensitive-word:query')")
    public CommonResult<SensitiveWordRespVO> getSensitiveWord(@RequestParam("id") Long id) {
        SystemSensitiveWord sensitiveWord = sensitiveWordService.getSensitiveWord(id);
    ...
    public CommonResult<PageResult<SensitiveWordRespVO>> getSensitiveWordPage(@Valid SensitiveWordPageReqVO pageVO) {
        return success(sensitiveWordService.getSensitiveWordPage(pageVO));
    }

    @GetMapping("/export-excel")
    @Operation(summary = "导出敏感词 Excel")
    @PreAuthorize("@ss.hasPermission('infra:data:sensitive-word:export')")
    @OperateLog(type = EXPORT)
    public void exportSensitiveWordExcel(@Valid SensitiveWordExportReqVO exportReqVO,
              HttpServletResponse response) throws IOException {
        List<SystemSensitiveWord> list = sensitiveWordService.getSensitiveWordList(exportReqVO);
        // 导出 Excel
