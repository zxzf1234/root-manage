...
public class NotifySendServiceImpl implements NotifySendService {
    ...
    public Long sendSingleNotify(Long userId, Integer userType, String templateCode, Map<String, Object> templateParams) {
        // 校验模版
        SystemNotifyTemplate template = validateNotifyTemplate(templateCode);
        if (Objects.equals(template.status(), CommonStatusEnum.DISABLE.getStatus())) {
        ...
        // 发送站内信
        String content = notifyTemplateService.formatNotifyTemplateContent(template.content(), templateParams);
        return notifyMessageService.createNotifyMessage(userId, userType, template, content, templateParams);
    }

    @VisibleForTesting
    public SystemNotifyTemplate validateNotifyTemplate(String templateCode) {
        // 获得站内信模板。考虑到效率，从缓存中获取
        SystemNotifyTemplate template = notifyTemplateService.getNotifyTemplateByCodeFromCache(templateCode);
        // 站内信模板不存在
        if (template == null) {
     ...
     */
    @VisibleForTesting
    public void validateTemplateParams(SystemNotifyTemplate template, Map<String, Object> templateParams) {
