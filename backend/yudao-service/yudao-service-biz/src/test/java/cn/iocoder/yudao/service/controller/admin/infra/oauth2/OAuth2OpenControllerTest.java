...
public class OAuth2OpenControllerTest extends BaseMockitoUnitTest {
    ...
    public void testPostAccessToken_authorizationCode() {
        ...
        // mock 方法（client）
        SystemOauth2Client client = SystemOauth2ClientDraft.$.produce(systemOauth2ClientDraft ->{
            randomPojo(SystemOauth2ClientDraft.class).setClientId("test_client_id");
        });
        ...
        SystemOauth2AccessToken accessTokenDO =  SystemOauth2AccessTokenDraft.$.produce(SystemOauth2AccessToken->{
            randomPojo(SystemOauth2AccessTokenDraft.class)
    ...
    public void testPostAccessToken_password() {
        ...
        HttpServletRequest request = mockRequest("test_client_id", "test_client_secret");
        // mock 方法（client）
        SystemOauth2Client client = SystemOauth2ClientDraft.$.produce(SystemOauth2Client->{
            randomPojo(SystemOauth2ClientDraft.class).setClientId("test_client_id");
        });
        ...
        // mock 方法（访问令牌）
        SystemOauth2AccessToken accessTokenDO = SystemOauth2AccessTokenDraft.$.produce(SystemOauth2AccessToken->{
            randomPojo(SystemOauth2AccessTokenDraft.class).setExpiresTime(LocalDateTimeUtil.offset(LocalDateTime.now(), 30000L, ChronoUnit.MILLIS));
        });
    ...
    public void testRevokeToken() {
        ...
        // mock 方法（client）
        SystemOauth2Client client = SystemOauth2ClientDraft.$.produce(SystemOauth2Client->{
            randomPojo(SystemOauth2ClientDraft.class).setClientId("demo_client_id");
        });
    ...
    public void testCheckToken() {
        ...
        // mock 方法
        SystemOauth2AccessToken accessTokenDO = SystemOauth2AccessTokenDraft.$.produce(SystemOauth2AccessToken->{
            randomPojo(SystemOauth2AccessTokenDraft.class).setUserType(UserTypeEnum.ADMIN.getValue()).setExpiresTime(LocalDateTimeUtil.of(1653485731195L));
        });
    ...
    public void testAuthorize() {
        ...
        // mock 方法（client）
        SystemOauth2Client client = SystemOauth2ClientDraft.$.produce(SystemOauth2Client->{
            randomPojo(SystemOauth2ClientDraft.class).setClientId("demo_client_id").setScopes(ListUtil.toList("read", "write", "all"));
        });
        when(oauth2ClientService.validOAuthClientFromCache(eq(clientId))).thenReturn(client);
        // mock 方法（approve）
        List<SystemOauth2Approve> approves = asList(
                SystemOauth2ApproveDraft.$.produce(SystemOauth2Approve->{
                    randomPojo(SystemOauth2ApproveDraft.class).setScope("read").setApproved(true);
                }),
                SystemOauth2ApproveDraft.$.produce(SystemOauth2Approve->{
                    randomPojo(SystemOauth2ApproveDraft.class).setScope("write").setApproved(false);
                })
        );
    ...
    public void testApproveOrDeny_autoApproveNo() {
        ...
        String state = randomString();
        // mock 方法
        SystemOauth2Client client = randomPojo(SystemOauth2Client.class);
    ...
    public void testApproveOrDeny_autoApproveWithToken() {
        ...
        // mock 方法（client)
        SystemOauth2Client client = SystemOauth2ClientDraft.$.produce(SystemOauth2Client->{
            randomPojo(SystemOauth2ClientDraft.class).setClientId(clientId).setAdditionalInformation(null);
        });
        when(oauth2ClientService.validOAuthClientFromCache(eq(clientId), isNull(), eq("implicit"),
                eq(asSet("read", "write")), eq(redirectUri))).thenReturn(client);
        // mock 方法（场景一）
        ...
        // mock 方法（访问令牌）
        SystemOauth2AccessToken accessTokenDO = SystemOauth2AccessTokenDraft.$.produce(SystemOauth2AccessToken->{
            SystemOauth2AccessToken.setAccessToken("test_access_token").setExpiresTime(LocalDateTimeUtil.offset(LocalDateTime.now(), 30010L, ChronoUnit.MILLIS));
        });
