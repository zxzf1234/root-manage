...
// reactive()的用法与 ref()的用法相似，也是将数据变成响应式数据 不同的是 ref()用于基本数据类型，
// 而 reactive()是用于引用类型的数 据，比如对象和数组。
...
const open = async (row: RoleApi.RoleVO) => {
  ...
  // 设置数据
  formData.id = row.id // 角色编号

  formData.name = row.name //角色名称
  formData.code = row.code // 角色标识
